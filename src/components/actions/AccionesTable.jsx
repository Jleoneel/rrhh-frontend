import { Eye, Edit, FileText, Download, MoreVertical, Filter } from "lucide-react";
import EstadoBadge from "./EstadoBadge";
import { useState } from "react";

export default function AccionesTable({ acciones, onView, onEdit, onDownload }) {
      ...
      <div className="px-6 py-4 border-b border-gray-200 bg-gradient-to-r from-gray-50 to-white">
        <div className="flex flex-col md:flex-row items-center justify-between gap-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-xl font-bold text-gray-900">Acciones de Personal</h2>
              <p className="text-sm text-gray-500">
                {acciones.length} acciones encontradas • {selectedRows.size} seleccionadas
              </p>
            </div>
          </div>
          ...
          <div className="flex items-center gap-3">
            {selectedRows.size > 0 && (
              <div className="bg-blue-50 text-blue-700 px-4 py-2 rounded-lg border border-blue-200">
                <span className="font-medium">{selectedRows.size} seleccionadas</span>
              </div>
            )}
            ...
            <tr>
              <th className="px-6 py-4">
                <div className="flex items-center">
                  <input
                    type="checkbox"
                    checked={selectedRows.size === acciones.length && acciones.length > 0}
              ...
              <tr>
                <td colSpan="7" className="px-6 py-12 text-center">
                  <div className="flex flex-col items-center justify-center text-gray-400">
                    ...
                    <p className="text-lg font-medium text-gray-500">No se encontraron acciones</p>
                    <p className="text-sm mt-1">Intenta con otros filtros o crea una nueva acción</p>
                  </div>
                </td>
              </tr>
            ) : (
              sortedAcciones.map((accion) => {
                ...
                return (
                  ...
                  >
                    ...
                    <td className="px-6 py-4">
                      <div className="flex flex-col">
                        <span className="font-medium text-gray-900">
                          {new Date(accion.fecha_elaboracion).toLocaleDateString('es-ES', {
                            day: '2-digit',
                            month: 'short',
                            year: 'numeric'
                          })}
                    ...
                    <td className="px-6 py-4">
                      <div className="font-mono text-gray-800">{accion.cedula}</div>
                    </td>
        ...
        <div className="px-6 py-4 border-t border-gray-200 bg-gray-50">
          <div className="flex flex-col md:flex-row items-center justify-between gap-4">
            <div className="text-sm text-gray-600">
              Mostrando <span className="font-semibold">{sortedAcciones.length}</span> de{" "}
              <span className="font-semibold">{sortedAcciones.length}</span> acciones
            </div>
            
            <div className="flex items-center gap-4">
              <div className="flex items-center gap-2">
                <span className="text-sm text-gray-600">Acciones por página:</span>
              ...
              <div className="flex items-center gap-2">
                ...
                <span className="px-3 py-1.5 bg-blue-600 text-white rounded-lg text-sm font-medium">1</span>
                <button className="px-3 py-1.5 border border-gray-300 rounded-lg text-sm hover:bg-gray-100">
                  Siguiente
                </button>
              </div>
